...
## 4) Random seed and train-test split in 80-20 ratio
***
```{r}
set.seed(123)
train_index <- createDataPartition(df$ceb, p = 0.8, list = F)
...
formula <- ceb ~ urban*educ + poly(age, 2) + urban*electric + agefbrth + agefm + educ*heduc + spirit + protest + catholic
...
```{r}
prediction <- predict(dev_model_glmnet, newdata = test_data)
pred_poisson <- predict(dev_model_poisson, newdata = test_data)
...
rmse_poisson <- sqrt(mean((results$Actual - results$Predicted_Poisson)^2))
print(paste("RMSE: ", rmse))
print(paste("RMSE_Poisson: ", rmse_poisson))
print(results)
```
...
## 9) Comparison to baseline assumption
***
```{r}
print(paste("RMSE: ", rmse))
print(paste("RMSE_Poisson: ", rmse_poisson))
...
#### The Poisson model's RMSE equals 1.56, i.e. on average the model is off by 1.56 "ever born children". Given the complex social, cultural and economic factors that influence fertility, the model has a moderate level of predictive accuracy, but also highlights important unexplained variation. The standard deviation of the dependent variable (ceb) is 2.31. This indicates that the model performs better than a naive baseline prediction that just guesses the mean number of ever born children for all observations.
